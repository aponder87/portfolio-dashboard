...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def compute_trade_records():
        ...
        # Pre-compute lookup columns for the holding-aware performance mode.
        #   • Next Complete Sale Date / Next New Buy Date — next events of each type
        #   • Episode Closed / Episode End Date — does this trade belong to a holding
        #     episode that was fully closed (ended in a Complete Sale), and if so when?
        # A "holding episode" begins with a New Buy (qty 0 → positive) and ends with a
        # Complete Sale (qty → 0). The most recent episode is "open" if the stock is still held.
        if not records_df.empty:
            records_df = records_df.sort_values(["Company", "Trade Date"]).reset_index(drop=True)
            records_df["Next Complete Sale Date"] = pd.NaT
            records_df["Next New Buy Date"] = pd.NaT
            records_df["Episode Closed"] = False
            records_df["Episode End Date"] = pd.NaT

            for co in records_df["Company"].unique():
                co_idx = list(records_df.index[records_df["Company"] == co])

                # Next-event lookups
                for i, idx in enumerate(co_idx):
                    future = records_df.loc[co_idx[i+1:]]
                    cs = future[future["Transaction Type"] == "Complete Sale"]
                    nb = future[future["Transaction Type"] == "New Buy"]
                    if not cs.empty:
                        records_df.at[idx, "Next Complete Sale Date"] = cs["Trade Date"].iloc[0]
                    if not nb.empty:
                        records_df.at[idx, "Next New Buy Date"] = nb["Trade Date"].iloc[0]

                # Episode grouping — walk trades, close an episode on each Complete Sale
                episode_trades = []
                for idx in co_idx:
                    episode_trades.append(idx)
                    if records_df.at[idx, "Transaction Type"] == "Complete Sale":
                        end_date = records_df.at[idx, "Trade Date"]
                        for ei in episode_trades:
                            records_df.at[ei, "Episode Closed"] = True
                            records_df.at[ei, "Episode End Date"] = end_date
                        episode_trades = []
                # Any trades left in episode_trades belong to the current (open) holding
    ...
    with f2:
        ...
        perf_mode = st.radio(
            "Performance mode",
            ["Holding-aware (recommended)", "End of selected date range",
             "Latest available", "Fixed days after trade"],
            index=0,
            label_visibility="collapsed",
            help=(
                "**Holding-aware (recommended)** — reflects what actually happened to each "
                "trading decision:\n"
                "- **New Buy** → only shown for stocks you **still hold** (measured to today). "
                "Buys that were later fully sold are hidden, so closed positions don't show "
                "flattering 'paper' gains you never captured.\n"
                "- **Addition** → measured to the **Complete Sale date** if the stock was later "
                "fully sold, otherwise to today.\n"
                "- **Complete Sale** → measured **from the sale date forward** (did the stock keep "
                "rising = left on table, or fall = good exit?).\n"
                "- **Partial Sale** → measured to today.\n\n"
                "**End of selected date range / Latest available / Fixed days** — measure every "
                "trade to a common endpoint regardless of holding status. Useful for evaluating "
                "raw entry timing in isolation."
            ),
        )
        fixed_days = None
    ...
    if perf_mode == "End of selected date range":
        ...
        endpoint = None  # used for "Fixed days" and "Holding-aware" modes
    ...
    for _, r in filtered.iterrows():
        ...
        elif perf_mode == "Holding-aware (recommended)":
            tx_type = r["Transaction Type"]
            episode_closed = bool(r.get("Episode Closed", False))
            episode_end = r.get("Episode End Date")

            if tx_type == "New Buy":
                if episode_closed:
                    # This buy was eventually fully sold — hide it (don't flatter closed positions)
                    perfs.append(np.nan); end_used.append(pd.NaT); continue
                target = AS_OF  # current holding → measure to today
            elif tx_type == "Addition":
                # to the complete sale date if the position was later fully sold, else to today
                target = episode_end if (episode_closed and pd.notna(episode_end)) else AS_OF
            elif tx_type == "Complete Sale":
                # out-of-position view: from the sale date forward to next New Buy, or today
                next_nb = r.get("Next New Buy Date")
                target = next_nb if pd.notna(next_nb) else AS_OF
            else:  # Partial Sale → always to today
                target = AS_OF
    ...
    if perf_mode == "End of selected date range":
        ...
        st.markdown(
            "_**New Buys** shown only for stocks still held (to today); buys later fully sold are hidden. "
            "**Additions** measured to the complete-sale date if later sold, else to today. "
            "**Complete Sales** measured from the sale date forward. "
            "**Partial Sales** measured to today._"
        )
    else:
