...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ── Compute per-trade records (cached) ─────────────────────────────────
    @st.cache_data(show_spinner="Building trade records...")
    def compute_trade_records(_pkl_mtime: float):
        """
        ...
        records = []
        instrument_qty = D["instrument_qty"]   # pre-built, split-adjusted
        ...
        for _, row in agg.iterrows():
            ...
            qty_series = instrument_qty.get(inst)
    ...
    trade_records, prices_df = compute_trade_records(DATA_PATH.stat().st_mtime)
    ...
    with f2:
        ...
        perf_mode = st.radio(
            "Performance mode",
            ["End of selected date range", "Latest available", "Fixed days after trade"],
            label_visibility="collapsed",
        )
        fixed_days = None
