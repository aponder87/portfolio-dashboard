...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    def compute_trade_records():
        ...
        # Pre-compute "next event date" columns for the "Until next trade" performance mode.
        # We track two specific event types only — the framework is restricted to full
        # entry/exit cycles:
        #   • Next Complete Sale Date — used as endpoint for New Buys (full holding period)
        #   • Next New Buy Date       — used as endpoint for Complete Sales (full out-of-position period)
        # Additions and Partial Sales fall back to AS_OF in the perf computation, since
        # they don't represent full entry or exit decisions.
        if not records_df.empty:
            records_df = records_df.sort_values(["Company", "Trade Date"]).reset_index(drop=True)
            records_df["Next Complete Sale Date"] = pd.NaT
            records_df["Next New Buy Date"] = pd.NaT
            for co in records_df["Company"].unique():
                co_idx = records_df.index[records_df["Company"] == co]
    ...
    with f2:
        ...
        perf_mode = st.radio(
            ...
            help=(
                ...
                "Applied only to full entry/exit cycles:\n"
                "- **New Buy** → measured to the next Complete Sale (or today if never sold)\n"
                "- **Complete Sale** → measured to the next New Buy (or today if never re-entered)\n\n"
                "**Additions and Partial Sales** fall back to today's price since they don't "
                "represent full entry or exit decisions.\n\n"
    ...
    for _, r in filtered.iterrows():
        ...
        if perf_mode == "Fixed days after trade":
            ...
            # The 'until next trade' framework only applies cleanly to full entry/exit cycles.
            #   • New Buy       → measure to the next Complete Sale (full holding period)
            #   • Complete Sale → measure to the next New Buy (full out-of-position period)
            #   • Addition / Partial Sale → fall back to AS_OF (latest available)
            if tx_type == "New Buy":
                next_cs = r.get("Next Complete Sale Date")
                target = next_cs if pd.notna(next_cs) else AS_OF
            elif tx_type == "Complete Sale":
                next_nb = r.get("Next New Buy Date")
                target = next_nb if pd.notna(next_nb) else AS_OF
            else:  # Addition or Partial Sale — use latest available
                target = AS_OF
            ep = _price_on_or_before(s, target)
            end_used.append(target)
        else:
    ...
    if perf_mode == "End of selected date range":
        ...
        st.markdown(
            "_Performance measured for full entry/exit cycles only: "
            "**New Buys** → to the next Complete Sale (or today). "
            "**Complete Sales** → to the next New Buy (or today). "
            "**Additions** and **Partial Sales** fall back to today's price._"
        )
    else:
